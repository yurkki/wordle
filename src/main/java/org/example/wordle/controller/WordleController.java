...
public class WordleController {
    ...
    /**
     * Переключить режим игры (API endpoint без перезагрузки)
     */
    @PostMapping("/api/switch-mode")
    @ResponseBody
    public Map<String, Object> switchModeApi(@RequestParam String mode, HttpSession session, HttpServletRequest request, HttpServletResponse response) {
        Map<String, Object> responseMap = new HashMap<>();
        
        try {
            System.out.println("=== API Switch Mode Request ===");
            ...
            if (mode == null || mode.trim().isEmpty()) {
                System.err.println("Mode parameter is null or empty");
                responseMap.put("success", false);
                responseMap.put("error", "Режим не указан");
                return responseMap;
            }
            ...
            try {
                ...
                System.out.println("Parsed game mode: " + gameMode);
            } catch (IllegalArgumentException e) {
                System.err.println("Invalid game mode: " + mode);
                responseMap.put("success", false);
                responseMap.put("error", "Неверный режим игры: " + mode);
                return responseMap;
            }
            ...
            System.out.println("Game mode: " + newGame.getGameMode());
            System.out.println("Target word: " + newGame.getTargetWord());
            System.out.println("Player ID set: " + newGame.getPlayerId());
            
            // Подготавливаем ответ
            responseMap.put("success", true);
            responseMap.put("gameState", newGame);
            responseMap.put("mode", gameMode.name());
            responseMap.put("todayDate", dailyWordService.getTodayDateString());
            responseMap.put("gameNumber", dailyWordService.getTodayGameNumber());
            
            System.out.println("Response prepared successfully");
            System.out.println("=== API Switch Mode Success ===");
            
        } catch (Exception e) {
            System.err.println("=== API Switch Mode Error ===");
            ...
            responseMap.put("success", false);
            responseMap.put("error", "Ошибка при переключении режима: " + e.getMessage());
        }
    ...
    /**
     * Переключить режим игры (старый endpoint для совместимости)
     */
